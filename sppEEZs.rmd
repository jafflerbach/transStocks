...
###(3) Rasterize EEZs (OHI regions) to the half degree cells, same as aquamaps. With rgn_id as zones
...
###(4) Rasterize each aquamap species, and then run zonal on each and get total number of eezs each species has 
...
#write.csv(df,file='intData/sp_probs_eez-preJoin.csv')
...
df = read.csv('intData/sp_probs_eez-preJoin.csv',stringsAsFactors=F)%>%
        dplyr::select(-X)
